...
from bots import botsglobal, botslib, models
...
def posttest(routestorun):
    """Compare run results"""
    # comparerunresults({'status':0,'lastreceived':33,'lasterror':0,'lastdone':33,'lastok':0,'lastopen':0,'send':62,'processerrors':0,'filesize':138045})
    result_expect = {
        'status': 0,
        'lastreceived': 33,
        'lasterror': 0,
    ...
    if 'confirmasked' not in [v[0] for v in models.RULETYPE]:
        # bots.__version__ < '3.8.6'
        # result_expect['send'] = 6
        pass

    comparerunresults(result_expect)
    ...
    botssys = botsglobal.ini.get('directories', 'botssys')
    cmpobj = filecmp.dircmp(
        os.path.join(botssys, 'outfile'),
        os.path.join(botssys, 'infile/outfile'))
