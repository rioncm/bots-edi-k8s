...
def my_preprocess_function(ta_from, endstatus, *args, **kwargs):
    ''' 
        Problem here is that the edi-file starts with a number of '#' and '@'.
        This function removes these.
    '''
    content = botslib.readdata(filename=ta_from.filename, charset='iso-8859-1')  # read content of incoming file.
    ...
    tofile = botslib.opendata(tofilename, 'wb', charset='iso-8859-1')  # open file for converted data.
    ...
    botsglobal.logger.debug('        File written: "%s".', tofilename)  # do logging
