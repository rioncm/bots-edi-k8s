...
from bots import botsglobal, botslib
...
def comparedicts(result_expect,result_run):
    ...
    if error:
        print(f"NOK: differences in run results: \n{error}\n")
    else:
...
def posttest(routestorun):
    # Compare run results
    comparerunresults({'status':0,'lastreceived':2,'lasterror':0,'lastdone':2,'lastok':0,'lastopen':0,'send':2,'processerrors':0,'filesize':630})
    ...
    if not os.path.isdir(outdir):
        print("Error: Output directory not created:", outdir)
        return False
    cmpobj = filecmp.dircmp(outdir, os.path.join(botssys, "infile/outfile"))
