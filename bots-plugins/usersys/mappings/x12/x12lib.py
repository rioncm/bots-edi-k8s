def get_art_num(node, qualifier, start=6, end=25):
    """helper function to get art_nr with right qualifier in line segment (PO1, etc).
    """
    tag = node.record['BOTSID']
    list_searchable_elements = [
        ('%s%02d' % (tag, i), '%s%02d' % (tag, i + 1)) for i in range(start, end, 2)]
