...
Bots grammar lib
"""
# pylint: disable=invalid-name, missing-class-docstring, too-many-lines, line-too-long
# flake8: noqa:E501
...
class Grammar:
    """
    Class for translation grammar.
    A grammar contains the description of an edi-file;
    this is used in reading or writing an edi file.
    ...
    in a grammar 'structure' is a list of dicts describing the sequence and relationships
    between the record(group)s:
    ...
    def _init_restofgrammar(self):
        ...
        if self.syntax['has_structure']:
            ...
            try:
                self._dorecorddefs()
            except GrammarPartMissing:
                # basic checks on recordsdef - it is not there, or not a dict, etc.
                raise
            except Exception:
                # mark recorddefs as 'already read - with errors'
                self.recorddefs[ERROR_IN_GRAMMAR] = True
                raise
            # mark recorddefs as 'read and checked OK'
            self.recorddefs[ERROR_IN_GRAMMAR] = False
            # read structure
            ...
            try:
                ...
                # mark structure as 'already read - with errors'
                self.structure[0][ERROR_IN_GRAMMAR] = True
                raise
            # mark structure as 'read and checked OK'
            self.structure[0][ERROR_IN_GRAMMAR] = False
    ...
    def _dorecorddefs(self):
        ...
        if ERROR_IN_GRAMMAR in self.recorddefs:
            # recorddefs is checked already (in this run).
            if self.recorddefs[ERROR_IN_GRAMMAR]:
                # already did checks - and an error was found.
                raise GrammarError(
                    _(
                        'Grammar "%(grammar)s": recorddefs has error'
                        ' that is already reported in this run.'
                    ),
                    {'grammar': self.grammarname},
                )
            # already did checks - result OK! skip checks
            return
    ...
    def _dostructure(self):
        ...
        if len(self.structure) != 1:
            raise GrammarPartMissing(
                _(
                    ...
                    ' structure must have exactlty one root record.'
                ),
                {'grammar': self.grammarname},
            )
        if not isinstance(self.structure[0], dict):
            raise GrammarPartMissing(
                _(
                    'Grammar "%(grammar)s": in structure:'
                    ' expect a dict for root record, but did not find that.'
                ),
                {'grammar': self.grammarname},
            )

        if ERROR_IN_GRAMMAR in self.structure[0]:
            # structure is checked already (in this run).
            if self.structure[0][ERROR_IN_GRAMMAR]:
                # already did checks - and an error was found.
                raise GrammarError(
                    _(
                        'Grammar "%(grammar)s": structure has error'
                        ' that is already reported in this run.'
                    ),
                    {'grammar': self.grammarname},
                )
    ...
    def _checknestedcollision(self, structure, collision=None):
        ...
        for i in reversed(structure):
            ...
            if isa_safeheadersegment or i[MIN] == i[MAX]:
                ...
                # this can be handled umambigiously: no check needed
                pass
    ...
    # bots interpreters the format from the grammer;
    # left side are the allowed values; right side are the internal formats bots uses.
    # the list directly below are the default values for the formats,
    # subclasses can have their own list.
...
class csv(Grammar):
    def class_specific_tests(self):
        if self.syntax['noBOTSID'] and len(self.recorddefs) != 2:
            raise GrammarError(
                _(
                    'Grammar "%(grammar)s": if syntax["noBOTSID"]:'
                    ' there can be only one record in recorddefs.'
                ),
                {'grammar': self.grammarname},
            )
    ...
    defaultsyntax = {
        ...
        "strict_syntax_check": False,  # for strict checks: no spaces between records
        "strip_value": False,  # Strip field value of incoming document
        'triad': '',
...
class fixed(Grammar):
    def class_specific_tests(self):
        ...
        if self.nextmessageblock is not None and len(self.recorddefs) != 2:
            raise GrammarError(
                _(
                    'Grammar "%(grammar)s": if nextmessageblock:'
                    ' there can be only one record in recorddefs.'
                ),
                {'grammar': self.grammarname},
            )
    ...
    def _linkrecorddefs2structure(self, structure):
        ...
        for i in structure:
            ...
            for field in i[FIELDS]:
                if field[ID] == 'BOTSID':
                    if self.is_first_record:
                        # for first record: 1. determine start/end of BOTSID;
                        # this is needed when reading/parsing fixed records.
                        ...
                        # for non-first records: 2. check if start/end of BOTSID is the same for all records;
                        # this is needed to correctly parse fixed files.
                        if (
                                self.syntax['startrecordID'] != position_in_record
                            ...
                            raise GrammarError(
                                _(
                                    'Grammar "%(grammar)s", record %(key)s:'
                                    ' position and length of BOTSID should be equal'
                                    ' in all records.'
                                ),
            ...
            if self.syntax['noBOTSID']:
                # correct record-length if noBOTSID
...
class xml(Grammar):
    def class_specific_tests(self):
        if not self.syntax['envelope'] and self.syntax['merge']:
            raise GrammarError(
                _(
                    'Grammar "%(grammar)s": in this xml grammar merge is "True"'
                    ' but no (user) enveloping is specified.'
                    ' This will lead to invalid xml files'
                ),
                {'grammar': self.grammarname},
            )
...
class templatehtml(Grammar):
    defaultsyntax = {
        ...
        'envelope-template': '',
        'merge': True,
        "template_engine": "genshi",  # genshi or django
        # settings needed as defaults, but not useful for this editype
        'add_crlfafterrecord_sep': '',
        'checkunknownentities': True,
        'escape': '',
...
class x12(Grammar):
    defaultsyntax = {
        ...
        'merge': True,
        'record_sep': '~',
        # if separator found, replace by this character;
        # if replacechar is an empty string: raise error
        'replacechar': '',
        'reserve': '^',
        'sfield_sep': '>',  # advised '\'?
        'skip_char': '\r\n',
        ...
        'ISA05': '01',
        'ISA07': '01',
        # since ISA version 00403 this is the reserve/repetition separator.
        # Bots does not use this anymore for ISA version >00403
        'ISA11': 'U',
        'ISA14': '0',
        'ISA15': 'P',
        'GS07': 'X',
        ...
        'triad': '',
        # bots internal, never change/overwrite
        # is True, read structure, recorddef, check these
        'has_structure': True,
        'checkcollision': True,
        'lengthnumericbare': True,
        'stripfield_sep': True,
...
class json(Grammar):
    defaultsyntax = {
        ...
        "force_list": False,  # outgoing. when True: max 1: object, max > 1
        "json_write_numericals": False,  # outgoing. when True: write numerical field types (N, R) to json types (int or float).
