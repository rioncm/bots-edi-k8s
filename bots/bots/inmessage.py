...
class fixed(Inmessage):
    ...
    def _parsefields(self, lex_record, record_definition) -> dict:
        ...
        record2build = {}
        # shortcut to fixed incoming record
        fixedrecord = lex_record[ID][FIXEDLINE]
...
class var(Inmessage):
    ...
    def _lex(self):
        """
        lexes file with variable records to list of lex_records,
            ...
            if not mode_inrecord:
                ...
                if char.isspace():
                    ...
                    if char in field_sep and isinstance(self, csv):
                        # exception for tab-delimited csv/excel files: if first field is not filled: first TAB is significant!
                        # just go on lexing
                        pass
                    elif strict_syntax_check:
    ...
    def _parsefields(self, lex_record, record_definition) -> dict:
        ...
        # ~ tsubindex = 0     #sub-element counter within composite; for files that are OK:
        # init when compostie is detected.
...
class excel(csv):
    ...
    def tupledate_to_isodate(self, tupledate):
        ...
        into a standard YYYY-MM-DDTHH:MM:SS ISO date.
        """
        (y, m, d, hh, mm, ss) = tupledate

        def nonzero(num) -> bool:
    ...
    # -------------------------------------------------------------------------------
    def utf8ize(self, data: list) -> list:
        """Make string-like things into utf-8, leave other things alone"""
        return [str(s).encode("utf-8") if hasattr(s, "encode") else s for s in data]
...
class edifact(var):
    ...
    @staticmethod
    def _manipulatemessagetype(messagetype, inode):  # pylint: disable=unused-variable
        """default: just return messagetype."""
    ...
    def _sniff(self):
        ...
        if (beginning of) edifact file is not correct: error.
        edifact file is read as binary. edifact have several charsets (UNOA, UNOC, UNOY).
        in processing is assumed: charset is ascii,
        uft-8 or some charset where 1char=1byte (eg iso-9959-1)
        (if other charset: would be easy to change. charset is in grammar,
        read/decode for charset, do parsing)
        Bots assumes: UNA-string contains NO extra CR/LF. (would be absurd; combination of:
        multiple UNA in file & using 'blocked' edifact.)
        """
...
class xml(Inmessage):
    ...
    def initfromfile(self):
        ...
        if self.ta_info['messagetype'] == 'mailbag':
            ...
            # containing python dicts.
            # Dict consist of 'xpath', 'messagetype' and (optionally) 'content'.
            # 'xpath' is a xpath to use on xml-file (using elementtree xpath functionality)
            # if found, and 'content' in the dict;
            # if 'content' is equal to value found by xpath-search, then set messagetype.
            ...
            try:
                ...
                # there is no extra_character_entity in the mailbag definitions, is OK.
                pass
        ...
        self.stackinit()
        # convert etree to bots-nodes-tree
        self.root = self._etree2botstree(etreeroot)
        self.checkmessage(self.root, self.defmessage)
        self.ta_info.update(self.root.queries)
    ...
    def _etree2botstree(self, xmlnode):
        ...
        for xmlchildnode in xmlnode:
            ...
            if not entitytype:
                # is a field, or unknown that looks like a field
                if xmlchildnode.text:
                    # if xml element has content, add as field
                    # add as a field
                ...
                # childnode is a record according to grammar
    ...
    def _etreenode2botstreenode(self, xmlnode):
        """
        build a basic dict from xml-node.
    ...
    def _entitytype(self, xmlchildnode):
        ...
        if LEVEL in structure_level:
            for structure_record in structure_level[LEVEL]:
    ...
    def stackinit(self):
        ...
        self.stack = [self.defmessage.structure[0]]
...
class json(Inmessage):
    ...
    def _dojsonlist(self, jsonobject, name):
        ...
        lijst = []
        # mostly a list will be of dicts (repeating group).
        # But it can happen it is a list of data-elements (int, string)
        is_repeting_data_element = False
        for i in jsonobject:
            if isinstance(i, dict):
                # check list item is dict/object
