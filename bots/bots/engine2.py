...
Translation information (as from translation table: mapping script, outgoing editype etc)
is eithr hard-coded now, or via translate table.

invalid-name, too-many-lines, too-many-branches, too-many-statements, missing-class-docstring,
missing-function-docstring, attribute-defined-outside-init, consider-using-f-string
"""
# pylint: disable=invalid-name, attribute-defined-outside-init, broad-exception-caught, broad-exception-raised
# pylint: disable=too-many-branches, too-many-statements, missing-class-docstring, missing-function-docstring, duplicate-code, line-too-long
# flake8: noqa:E501
...
from .engine import logging_exit
from .exceptions import BotsError, BotsImportError, CommunicationOutError, txtexc
from .transform import handle_out_message
...
def abspathdata(filename):
    """
    abspathdata if filename incl dir: return absolute path;
    else (only filename): return absolute path (datadir).
...
def start():
    ...
    Does the actual translations and communications;
    ...
    # advantage: when using relative paths it is clear that this point paths within bots installation.
    ...
    try:
        ...
        botsglobal.logger.exception(
            _(
                ...
                ' Database settings are in bots/config/settings.py. Error: "%(exc)s".'
            ),
            {'exc': exc},
        )
        sys.exit(1)
...
def get_control_information():
    ...
    run.translation = {"editype": "edifact", "messagetype": "edifact"}
    # ~ run.translation = {"editype": "edifact", "messagetype": "edifact", "tscript": "orders_edifact2xml", "toeditype": "xml", "tomessagetype": "orders"}
    return run
...
def read_incoming(run):
    ...
    for infilename in filelist:
        try:
            ...
            # move if to be delted
            shutil.copy(infilename, abs_filename)
        except Exception:
            txt = txtexc()
        else:
            txt = ''  # no errors
        ...
        try:
            ...
            for inn_splitup in edifile.nextmessage():  # splitup messages in parsed edifile
                try:
                    ...
                    while True:
                        # continue as long as there are (alt-)translations
                        ...
                        if 'topartner' not in out_translated.ta_info:
                        ...
                        if isinstance(doalttranslation, dict):
                            # some extended cases;
                            # a dict is returned that contains 'instructions' for some type of chained translations
...
def mergemessages(run):
    ...
    for env_criteria, rest_of_info in merge_yes.items():
        ...
        ta_info['filename'] = transform.unique('bots_file_name')
        ta_info['nrmessages'] = rest_of_info[2]
        # for reference: list of infilenames
        ta_info['infilename'] = rest_of_info[1]
        ta_info['error'] = ''
        try:
    ...
    for ta_info, filenames, infilenames in merge_no:
        ...
        ta_info['filename'] = transform.unique('bots_file_name')
        # for reference: list of infilenames
        ta_info['infilename'] = infilenames
        ta_info['error'] = ''
        try:
...
def filename_formatter(filename_mask, ta_info):
    ...
    unique = str(botslib.unique("bots_outgoing_file_name"))
    ...
    if '{' in tofilename:
        ...
        infilename = infilestr(os.path.basename(ta_info['infilename'][0]))
